...
function preencher_coisas(){
	...
	for(let i = 0; i < response.length; i++) {
		...
		// let aux_str = "#edit_date_regiao_6 > option";
		console.log(aux_str);
...
$.post("../../Controller/get_dates.php", {}).done(function(data){
	...
	for(let i = 0; i < response.length; i++){
		...
		let aux_str = "<form id='edit-date-" + this_response['id'] + "'>\n" +
			...
			"</select><div>\n" +
			"<button class='botao botao-medio' onclick='atualizar_date(" + this_response['id'] + ")' id='atualizarDate'>Atualizar date</button>\n" +
			"<button class='botao botao-medio' onclick='remover_date(" + this_response['id'] + ")' id='removerDate'>Remover date</button>\n" +
			"</div>" +
			"</form>";
